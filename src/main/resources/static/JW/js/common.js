...
/*
	...
	h		弹出层高度（缺省调默认值）
*/
function local_layer_show(title, url, w, h) {
    if (title == null || title == '') {
        title = false;
    };
    if (url == null || url == '') {
...
function local_layer_showAuto(title, url) {
    local_layer_show(title, url, '', '');
}
...
function createMenuItem(dataUrl, menuName) {
    ...
    $('.menuTab', window.parent.document).each(function() {
        if ($(this).data('id') == dataUrl) {
            if (!$(this).hasClass('active')) {
                ...
                scrollToTab(this);
                // 显示tab对应的内容区
                $('.mainContent .JW_iframe', window.parent.document).each(function() {
                    if ($(this).data('id') == dataUrl) {
                        $(this).show().siblings('.JW_iframe').hide();
                        return false;
                    }
                });
            }
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="JW_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
        $('.mainContent', window.parent.document).find('iframe.JW_iframe').hide().parents('.mainContent').append(str1);
