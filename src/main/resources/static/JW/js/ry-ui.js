...
$(function(){
	...
	$.modalMsg = function(content, type) {
	    if (type != undefined) {
	        ...
	        top.layer.msg(content, { icon: icon, time: 2000, shift: 0 });
	...
	$.modalConfirm = function (content, callBack) {
	       var layerIndex= top.layer.confirm(content, {
	        icon: 3,
	        title: "系统提示",
	        btn: ['确认', '取消'],
	...
	$.parentReload = function () {
		//parent.location.reload();
		//alert(parent.$('#content-main iframe:visible').length);
        parent.layer.closeAll();
        parent.$('#content-main iframe:visible').attr('src', parent.$('#content-main iframe:visible').attr('src'));
	    return false;
	};
	// 初始bootstrap table数据
