package com.JWmes.project.tool.gen.mapper;

import com.JWmes.project.tool.gen.entity.ColumnInfoEntity;
import com.JWmes.project.tool.gen.entity.TableInfoEntity;

import java.util.List;

/**
 * 代码生成 数据层
 * 
 * @author zhong
 */
public interface GenMapper
{
    /**
     * 查询ry数据库表信息
     * 
     * @param tableInfo 表信息
     * @return 数据库表列表
     */
    public List<TableInfoEntity> selectTableList(TableInfoEntity tableInfo);

    /**
     * 根据表名称查询信息
     * 
     * @param tableName 表名称
     * @return 表信息
     */
    public TableInfoEntity selectTableByName(String tableName);

    /**
     * 根据表名称查询列信息
     * 
     * @param tableName 表名称
     * @return 列信息
     */
    public List<ColumnInfoEntity> selectTableColumnsByName(String tableName);
}
