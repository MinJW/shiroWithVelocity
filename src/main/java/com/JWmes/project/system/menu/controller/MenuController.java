package com.JWmes.project.system.menu.controller;
...
public class MenuController extends BaseController
{
    ...
    public MessageEntity remove(@PathVariable("menuId") Long menuId)
    {
        ...
        {
            return MessageEntity.error( "存在子菜单,不允许删除");
        }
        if (menuService.selectCountRoleMenuByMenuId(menuId) > 0)
        {
            return MessageEntity.error( "菜单已分配,不允许删除");
        }
    ...
    @ResponseBody
    public MessageEntity checkMenuNameUnique(MenuEntity menu)
    {
        String uniqueFlag = "0";
        if (menu != null)
        {
    ...
    @ResponseBody
    public MessageEntity roleMenuTreeData(RoleEntity role)
    {
    ...
    @ResponseBody
    public  MessageEntity menuTreeData(RoleEntity role)
    {
